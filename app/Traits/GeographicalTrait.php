...
trait GeographicalTrait
{
    ...
    public function scopeDistance($query, $latitude, $longitude, $options = [])
    {
        ...
        $sql =
            "((ACOS(SIN(? * PI() / 180) * SIN(" .
            $latName .
            " * PI() / 180) + COS(? * PI() / 180) * COS(" .
            $latName .
            " * PI() / 180) * COS((? - " .
            $lonName .
            ") * PI() / 180)) * 180 / PI()) * 60 * ?) as distance";
    ...
    public function scopeGeofence(
        ...
        $query = $this->scopeDistance($query, $latitude, $longitude, $options);
        return $query->havingRaw("distance BETWEEN ? AND ?", [
            $inner_radius,
            $outer_radius,
        ]);
    }
